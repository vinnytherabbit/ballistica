...
class Bomb(bs.Actor):
    ...
    def __init__(
        ...
        *,
        ...
        # they may inherit the hit type and subtype of that source,
        # but only if the incoming subtype matches the bomb's own type.
        # This restores part of the old behavior (e.g., the "mine punch" trick)
        # in a safer and more controlled way, preventing unrelated type propagation.
        # Now, land-mines and impact bombs can behave like punches when appropriate,
        # without breaking interactions with objects they are meant to destroy.
    ...
    def _handle_hit(self, msg: bs.HitMessage) -> None:
        ...
        if not self._exploded and (
            ...
            if source_player is not None:
                ...
                # UPDATE (August 2025):
                # Only inherit hit type/subtype when the incoming subtype matches this bomb's type.
                # This restores part of the old 'mine punch' behavior in a controlled way,
                # allowing for custom damage logic (e.g. in spaz.py) to remain consistent.
                # Unlike before, this won't propagate punch effects across different bomb types.
                if self.bomb_type != 'tnt':
